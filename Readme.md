...
- Vector Embeddings are the `Heart of RAG`.

* Vector Embeddings capture the `Semantic or Contextual Information` from the data and arrange them in a **Vector space**.
* These Vector Embeddings are generated by **Text-Embedding model** which was offered by Azure Open API. Usually it is _ADA-002 with 1536 Dimensions_.

* High Dimensional Representation of Vector Embeddings
  ![Vector Embeddings](https://github.com/user-attachments/assets/1a148899-4fec-4e77-b38b-0b4ea79e0b38)

> Tokens -> the input strings are converted into tokens through Token Encoder

##### Storage Options for Vector Embeddings

1. Azure Cognitive(AI) search
2. Azure Cosmos DB
3. Azure Redis Cache (via Redis Vector Embeddings)

![Embedding Architecture](https://github.com/user-attachments/assets/2faf1293-8530-4754-af9c-dfe0839d32e0)

### RAG with Azure AI Search

- It is a Azure resource, we can give unstructured documents (ppt, xlsx, doc, pdf, txt) to it, and it can extract the images and contents from it and generate the captions for the images.
- It can extract data for **Multi Model data** (both texts and images).
- `Data Source:` Json Format (returns as json), Auto Crawling (bolb, azure cosmos).
- `Search and Analysis:` Vector Search, Full text, Hybrid search (both vector search and text matching search).
- `AI Encrichment:` Azure AI for Image
- `Linguistic Analysis:` 56 languages, Phonetic Matching, Azure AI Search, Bing, Office.
- `User Experience:` Query Syntax (Lucene Syntax like a SQL)
- `Azure Integration:` Scale(Scale in and out), Security(Entra identity)

![Azure AI Search](https://github.com/user-attachments/assets/a9f317f3-535e-46f6-ba51-94d06ec46995)
